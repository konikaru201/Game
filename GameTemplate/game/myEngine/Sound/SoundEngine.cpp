...
void CSoundEngine::Init()
{
	...
	{
		Release();
		//TK_ASSERT(false, "Faild GetDeviceDetails");
		return;
	}
	...
	{
		Release();
		//TK_ASSERT(false, "not enough channel");
		return;
	}
...
IXAudio2SourceVoice* CSoundEngine::CreateXAudio2SourceVoice(CWaveFile* waveFile, bool is3DSound)
{
	...
	if (is3DSound == false) {
		...
		sendDescriptors[0].pOutputVoice = /*SoundEngine().*/g_soundEngine->GetMasteringVoice();
		...
		sendDescriptors[1].pOutputVoice = /*SoundEngine().*/g_soundEngine->GetSubmixVoice();
...
void CSoundEngine::Update()
{
	...
	float deltaTime = Timer::GetFrameDeltaTime();
	if (deltaTime > 0.0f) {
		//リスナーの移動速度を計算する。
		D3DXVECTOR3 vel;
