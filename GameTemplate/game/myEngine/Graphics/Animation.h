...
public:
	...
	*/
	...
	/*!
	* @brief	アニメーションのループフラグを設定する。
	*@param[in]	animationSetIndex		アニメーションインデックス。
	*@param[in] loopflg					アニメーションの終了フラグ。
	*/
	void SetAnimationLoopflg(int animationSetIndex, bool loopflg )
	{
		if (animationSetIndex < numAnimSet) {
			animationLoopFlags[animationSetIndex] = loopflg;
		}
	}
	/*!
	*@brief	アニメーションの再生。
